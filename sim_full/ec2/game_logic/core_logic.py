...
from game_logic.anticheat import distance_between
    ...
    def _choose_ghost_step(self, ghost: dict, target_x: float, target_y: float):
        ...
        for scale in (1.0, 0.75, 0.5):
            ...
            for offset in offsets:
                ...
                move_dist = distance_between(current_x, current_y, next_x, next_y)
                if move_dist < 0.05:
                    continue
                remaining = distance_between(next_x, next_y, target_x, target_y)
    ...
    async def _check_bits(self):
        ...
        for i, bit in enumerate(self.state.bits):
            if not bit[2]:   # already collected
                continue
            dist = distance_between(rx, ry, bit[0], bit[1])
    ...
    async def _check_proximity(self):
        ...
        for i in range(len(players)):
            for j in range(i + 1, len(players)):
                ...
                dist = distance_between(p1["x"], p1["y"], p2["x"], p2["y"])
